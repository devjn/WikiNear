...
public class KMeans implements Runnable {
     ...
     * Constructor
     *
     * @param strings       array containing the strings to be clustered.
     ...
     */
    public KMeans(String[] strings, int k, int maxIterations, long randomSeed) {
        this.strings = strings;
        // Can't have more clusters than coordinates.
        mK = Math.min(k, strings.length);
    ...
    public Cluster getBiggestCluster() {
        int largest = 0;
        for (int i = 1; i < mClusters.length; i++) {
            if (mClusters[i].getMemberIndexes().length > mClusters[largest].getMemberIndexes().length) largest = i;
        }
        return mClusters[largest]; // position of the first largest found
    }
    ...
    private void initCenters() {
        ...
        for (int i = 0; i < mK; i++) {
            ...
            mProtoClusters[i] = new ProtoCluster(strings[coordIndex], coordIndex);
...
//            }
        ...
        for (int coord = 0; coord < numCoords; coord++) {
            ...
            for (int clust = 0; clust < numClusters; clust++) {
                ...
                if (cluster.getConsiderForAssignment() && cluster.needsUpdate()) {
                    mDistanceCache[coord][clust] = LevenshteinDistance.getDefaultInstance().
                            apply(strings[coord], cluster.getCenter());
                }
            }
        }
    ...
    private static class ProtoCluster {
         ...
         */
        ProtoCluster(String center, int coordIndex) {
            mCenter = center;
